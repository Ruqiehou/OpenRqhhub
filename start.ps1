...
catch { Write-Host "  [ERR] 请安装 Node.js: https://nodejs.org/" -ForegroundColor Red; Read-Host "回车退出"; exit 1 }
try { $v = git --version; Write-Host "  [OK] $v" -ForegroundColor Green }
catch { Write-Host "  [ERR] 请安装 Git: https://git-scm.com/" -ForegroundColor Red; Read-Host "回车退出"; exit 1 }
...
Write-Host "  启动完成 - 本地访问" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$root = $PSScriptRoot
# 后端绑定 localhost，前端注入 API 地址
Start-Process -WindowStyle Normal -FilePath "cmd.exe" -ArgumentList "/c title GitLocal-Backend && cd /d $root\backend && set HOST=localhost && npm run dev"
Start-Process -WindowStyle Normal -FilePath "cmd.exe" -ArgumentList "/c title GitLocal-Frontend && cd /d $root\frontend && set VITE_API_BASE_URL=http://localhost:3001/api && npx vite --host localhost"
