...
:: 启动前后端（独立窗口，绑定 localhost）
start "GitLocal-Backend" cmd /c "title GitLocal-Backend && cd /d %~dp0backend && set HOST=localhost && npm run dev"
start "GitLocal-Frontend" cmd /c "title GitLocal-Frontend && cd /d %~dp0frontend && set VITE_API_BASE_URL=http://localhost:3001/api && npx vite --host localhost"
